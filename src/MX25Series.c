...
MX25Series_status_enum_t MX25Series___issue_command(MX25Series_t *dev, MX25Series_COMMAND_enum_t command)
{
    ...
    return MX25Series_status_ok;
    #else /* !(TEST_HAL_API != 0) */
    HAL_StatusTypeDef status = HAL_SPI_Transmit(&hspi2, &issue_cmd, sizeof(issue_cmd), HAL_MAX_DELAY);
    if (status == HAL_OK)
    {
        return MX25Series_status_ok;
    }
...
__attribute__((weak)) MX25Series_status_enum_t MX25Series_read_electronic_signature(MX25Series_t *dev, int *electronic_id)
{
    MX25Series_status_enum_t result = MX25Series_status_init;
    uint8_t value[2] = {0};
    uint8_t issue_cmd[2] = {MX25Series_Command_REMS,0};

    MX25Series___enable_cs_pin(dev, true);
    uint8_t spi_port = *(uint8_t*)dev->ctx;
    if (hal__SPIWRITE_READ(spi_port, &value, &issue_cmd, 2) != 2)
    {
        result = MX25Series_status_error;
    }
    else
    {
        result = MX25Series_status_ok;
        *(uint8_t*)electronic_id = value;
    }
    #if 0
...
{
    assert(dev != NULL);
    HAL_GPIO_WritePin(GPIOA, 1 << dev->cs_pin, value ? GPIO_PIN_RESET : GPIO_PIN_SET);
}

void MX25Series___enable_reset_pin(MX25Series_t *dev, bool value)
{
    assert(dev != NULL);
    HAL_GPIO_WritePin(GPIOA, 1 << dev->reset_pin, value ? GPIO_PIN_RESET : GPIO_PIN_SET);
}

void MX25Series___enable_write_protect_pin(MX25Series_t *dev, bool value)
{
    assert(dev != NULL);
    HAL_GPIO_WritePin(GPIOA, 1 << dev->wp_pin, value ? GPIO_PIN_RESET : GPIO_PIN_SET);
}
