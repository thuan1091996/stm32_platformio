...
{
    if (HAL_SPI_GetState(&__spi_handle[0]) != HAL_SPI_STATE_READY)
    {
        ...
        */
        ...
        __spi_handle[0].Init.BaudRatePrescaler = SPI_BAUDRATEPRESCALER_256;
...
static int __initSPI2(void)
{
    if (HAL_SPI_GetState(&__spi_handle[1]) != HAL_SPI_STATE_READY)
    {
        ...
        */
        ...
        gpioinitstruct.Pull = GPIO_NOPULL;
        gpioinitstruct.Speed = GPIO_SPEED_FREQ_VERY_HIGH;
        gpioinitstruct.Alternate = GPIO_AF1_SPI2;
        ...
        __spi_handle[1].Init.BaudRatePrescaler = SPI_BAUDRATEPRESCALER_256;
...
// Write data to SPI device. Returns number of bytes written on success, -1 on failure. //Ignores data returned from SPI
// device.
...
int hal__SPIWRITE_READ(uint8_t spi_num, uint8_t *data_out, uint8_t *data_in, uint16_t len)
{
    ...
    HAL_StatusTypeDef status =
        HAL_SPI_TransmitReceive(&__spi_handle[spi_num - 1], data_in, data_out, len, SPI_DEFAULT_TIMEOUT);
    if (status == HAL_OK)
    {
        return len;
    }
